...
class GeoConnector:
	...
	def get_dist(self, start_pt, end_pt, mode):
		'''Get the distance from start_pt to end_pt. Mode must either be WALKING_MODE
		or DRIVING_MODE. If traveling between the two points is not possible, None is returned.
		...
		return float('%.2f' % travel_duration_mins), float('%.2f' % distance_miles)